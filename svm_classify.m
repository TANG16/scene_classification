...
num_of_categories = size(categories, 1);
num_of_training = size(train_image_feats, 1);
[num_of_tests, num_of_dim] = size(test_image_feats);
W = zeros(num_of_categories, num_of_dim);
B = zeros(num_of_categories, 1);
train_inv = train_image_feats';
test_inv = test_image_feats';
%LAMBDA = 0.0001;
num_of_iter = 10 / LAMBDA;
parfor i = 1 : num_of_categories
    labels = zeros(num_of_training,1) - 1;
    labels(strcmp(categories{i}, train_labels)) = 1;
    [w, b] = vl_svmtrain(train_inv, labels, LAMBDA, 'MaxNumIterations', num_of_iter);
    W(i,:) = w';
    B(i) = b;
end

[~, idx] = max(W * test_inv + repmat(B, 1, num_of_tests));
